...
public class GeoOperation {
    ...
    /**
     * Adds a member with a geospatial point to a sorted set stored at the specified
     * key.
     *
    ...
    /**
     * Calculates the distance between two members in a geospatial index stored at
     * the specified key.
     *
     ...
     */
    public Distance Dist(String key, String member1, String member2) {
     ...
     * @param member1 the first member
     * @param member2 the second member
     * @param metric  the metric to use for distance calculation (e.g.,
     *                {@link Metric#KILOMETERS}, {@link Metric#MILES})
     * @return the distance between the two members
     */
    public Distance Dist(String key, String member1, String member2, Metric metric) {
        return redisTemplate.opsForGeo().distance(key, member1, member2, metric);
    }

    /**
     * Retrieves the geohash strings representing the specified members in the given
     * key.
     *
    ...
    /**
     * Retrieves the position (longitude and latitude) of a member in a geospatial
     * index stored at the specified key.
     *
     * @param key    the key of the geospatial index
     * @param member the member whose position needs to be retrieved
     * @return a list of Point objects representing the longitude and latitude of
     *         the member
     */
    public java.util.List<Point> Pos(String key, String member) {
        return redisTemplate.opsForGeo().position(key, member);
    }

    /**
     * Returns the geospatial members that are within the given radius of the
     * specified key.
     *
     * @param key    the key to perform the radius operation on
     * @param within the circle representing the radius and center point
     * @return a {@link GeoResults} containing the geospatial members within the
     *         radius
     */
    ...
    /**
     * Returns the geospatial members that are within the given radius of the
     * specified key and circle.
     *
     * @param key    the key representing the geospatial index
     * @param within the circle representing the radius within which to search for
     *               members
     * @param args   additional arguments for the geo radius command
     * @return a {@link GeoResults} object containing the geospatial members within
    ...
    /**
     * Retrieves all the members within the specified radius from the given key and
     * member.
     *
     ...
     * @return a GeoResults object containing the members found within the specified
     *         radius
     */
    ...
    /**
     * Returns the geospatial members that are within the specified distance from
     * the given member in the sorted set stored at the specified key.
     *
     * @param key      the key of the sorted set
     * @param member   the member to calculate the distance from
     * @param distance the maximum distance from the member
     * @return a {@link GeoResults} containing the geospatial members and their
     *         distances
     */
    ...
    /**
     * Returns the geospatial members that are within the given distance from the
     * specified member.
     *
     * @param key      the key of the geospatial index
     * @param member   the member to calculate the distance from
     ...
     * @return a {@link GeoResults} containing the geo locations found within the
     *         circle
     */
    public GeoResults<RedisGeoCommands.GeoLocation<String>> Search(String key, Circle within) {
        return redisTemplate.opsForGeo().search(key, within);
    }

    /**
     * Searches for geo locations within a specified radius from a given reference
     * point.
     *
     * @param key       the key of the geo set
     * @param reference the reference point for the search
     * @param radius    the maximum distance from the reference point to search
     *                  within
     * @return a list of geo locations within the specified radius
     */
    public GeoResults<RedisGeoCommands.GeoLocation<String>> Search(String key, GeoReference<String> reference,
            Distance radius) {
     ...
     */
    public GeoResults<RedisGeoCommands.GeoLocation<String>> Search(String key, GeoReference<String> reference,
            Distance radius, RedisGeoCommands.GeoRadiusCommandArgs args) {
    ...
    /**
     * Searches for geo locations within the specified bounding box and returns the
     * results.
     *
     * @param key         the key of the geo set
     * @param reference   the reference point for the search
     ...
     */
    public GeoResults<RedisGeoCommands.GeoLocation<String>> Search(String key, GeoReference<String> reference,
            BoundingBox boundingBox, RedisGeoCommands.GeoRadiusCommandArgs args) {
    ...
    /**
     * Searches for members within a given radius and stores the results in a
     * destination key.
     *
     * @param key     the key representing the geo set to search within
     * @param destKey the key representing the destination geo set to store the
     *                results
    ...
    /**
     * Searches for geo coordinates within a specified radius and stores the results
     * in a destination key.
     *
     * @param key       the key to search for geo coordinates
     * @param destKey   the key to store the search results
     * @param reference the reference point for the search
     * @param radius    the maximum distance from the reference point to search for
     *                  geo coordinates
     * @return the number of geo coordinates found and stored in the destination key
     */
    public Long SearchAndStore(String key, String destKey, GeoReference<String> reference, Distance radius) {
     ...
     * @param destKey   the key representing the destination set to store the
     *                  results
     * @param reference the reference point for the search
     * @param radius    the maximum distance from the reference point to search
     *                  within
    ...
    /**
     * Searches for members within a specified bounding box in a sorted set stored
     * at the given key,
     ...
     */
    public Long SearchAndStore(String key, String destKey, GeoReference<String> reference, BoundingBox boundingBox) {
        return redisTemplate.opsForGeo().searchAndStore(key, destKey, reference, boundingBox);
    }

    /**
     * Searches for geo locations within a specified bounding box and stores the
     * results in a destination key.
     * 
     * @param key         the key representing the geo set to search within
     * @param destKey     the key to store the search results
     * @param reference   the reference point for the search
    ...
    /**
     * Searches for elements matching the given criteria and stores the results in a
     * destination key.
     *
     ...
     */
    public Long SearchAndStore(String key, String destKey, GeoReference<String> reference, GeoShape geoPredicate,
            RedisGeoCommands.GeoSearchStoreCommandArgs args) {
